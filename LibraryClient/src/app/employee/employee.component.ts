...
import { IBorrowedBookDto, IStatus } from "src/Dtos/BorrowedBook/IBorrowedBookDto";
...
export class EmployeeComponent implements OnInit {
  ...
  save():void{
    ...
    this.borrowingBookService.changeStatus(data).subscribe({next:resp=>{
      this.borrowedBooks[this.borrowedBooks.findIndex((x) => x.id == resp.id)] = resp;
      this.borrowedBookDialog=false;
      this.extraFunctions.showToast("success","sukces","Pomyślna zmiana statusu");
    },error:error=>{
      if (error === "Can't change returned book's status") {
        this.extraFunctions.showToast("error", "Błąd", "Ta książka zostałą już zwrócona");
      }
    }})
  }
  ...
  editStatus(id: number) {
    ...
    let bookR = this.borrowedBooks[this.borrowedBooks.findIndex((x) => x.id == id)];
